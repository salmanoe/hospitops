...
public class RoomTypeService implements ManageRoomTypeUseCase {
    ...
    public RoomTypeResponse createRoomType(CreateRoomTypeCommand cmd) {
        ...
        return RoomTypeResponse.from(roomTypeRepo.save(
            RoomType.create(cmd.name(), cmd.capacity(), cmd.description(), Money.of(cmd.basePrice()))));
    }
    ...
    public RoomTypeResponse findRoomTypeById(RoomTypeId id) { return RoomTypeResponse.from(findRoomType(id)); }
